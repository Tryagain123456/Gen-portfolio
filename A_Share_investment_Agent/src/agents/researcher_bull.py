...
def researcher_bull_agent(state: AgentState):
    ...
    """
    show_workflow_status("Bullish Researcher (LLM)")
    ...
    try:
        ...
        try:
            ...
            logger.error(msg)
            state["messages"].append(AIMessage(content=msg, name="researcher_bull_agent")) # type: ignore
            return state
    ...
    你的职责：
    1.  **放大正面信号**：如果数据（如基本面、情绪）是正面的，你要大力赞扬它们是强劲的增长动力。
    ...
    **输出格式 (必须严格遵守此JSON结构)：**
    ...
    try:
        ...
        if show_reasoning:
            show_agent_reasoning(f"LLM Raw Response:\n{structured_llm_response}", "Bullish Researcher (LLM)")

        # 确保 LLM 遵守了指令
        structured_llm_response["perspective"] = "bullish"

    except Exception as e:
        logger.error(f"Bullish Researcher LLM 失败或解析JSON错误: {e}")
    ...
    message = HumanMessage(
        ...
        name="researcher_bull_agent", # <-- 修改 Agent 名称
    )

    if show_reasoning:
        show_agent_reasoning(structured_llm_response, "Bullish Researcher") # <-- 修改 Agent 名称
        # 保存推理信息到metadata供API使用
        state["metadata"]["agent_reasoning"] = structured_llm_response

    show_workflow_status("Bullish Researcher", "completed") # <-- 修改 Agent 名称
    
    # (重要) 返回 state 时要确保 state["messages"] 确实被更新了
