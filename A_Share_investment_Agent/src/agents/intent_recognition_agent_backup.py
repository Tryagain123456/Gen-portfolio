...
from src.tools.openrouter_config import get_chat_completion # 调用大模型
...
# 定义意图识别agent: 分析对话历史, 提取股票代码等参数
def intent_recognition_agent(state: AgentState):
    # 1. 前置准备
    show_workflow_status("Intent Recognition Agent")  # 日志提示开始工作
    show_reasoning = state["metadata"]["show_reasoning"]  # 是否需要显示推理过程
    # 2. 获取用户输入文字性需求及现有data数据
    ...
    system_message = {
        "role": "system",
        "content": """
        你是一个股票代码识别工具，专门用于从用户输入中提取需要分析的A股股票，并返回其6位数字代码。
        示例：'大智慧' -> '601519', '贵州茅台' -> '600519'。
        请遵循以下规则：
        1. 仅关注用户明确提到的A股股票（忽略港股、美股等其他市场股票）。
        2. 优先识别股票名称（如"贵州茅台"）或简称（如"茅台"）对应的6位代码；若用户直接提供6位数字代码（如"600519"），则直接返回该代码。
        3. 若用户提到多只股票，仅返回第一只股票的6位代码。
        4. 输出仅返回6位数字代码（如"600519"），不添加任何额外解释或内容。
        """
    }
    user_message = {
        "role": "user",
        "content": f"需要分析的问题须在下述文字中提取：\n\n{user_input}\n\n请以JSON格式返回结果，其中字段名为ticker"
    }
    ## 尝试解析
    ticker_result = None
    max_attempts = 5
    attempts = 0
    while attempts < max_attempts and ticker_result is None:
        attempts += 1
        try:
            # 调用LLM获取结果
            result = get_chat_completion([system_message, user_message])
            # 尝试解析JSON
            ticker_result = json.loads(result.strip())
            ## 验证结果格式是否正确
            if not isinstance(ticker_result, dict) or "ticker" not in ticker_result:
                raise ValueError("结果缺少'ticker'字段")
            ## 验证股票代码格式（6位数字）
            if not re.match(r'^\d{6}$', ticker_result["ticker"]):
                raise ValueError("股票代码格式不正确，应为6位数字")
        except json.JSONDecodeError:
            # 尝试提取代码块中的JSON
            json_match = re.search(r'```json\s*(.*?)\s*```', result, re.DOTALL) # type: ignore
            if json_match:
                ticker_result = json.loads(json_match.group(1).strip())
    # 4.
    if ticker_result and "ticker" in ticker_result:
        show_workflow_status("Intent Recognition Agent", "completed")

    return {
        "messages": state["messages"],
        "data": {
            **data,
            "ticker": ticker_result['ticker'] # type: ignore
        },
