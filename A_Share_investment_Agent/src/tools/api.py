...
#         cap_series = ak.stock_value_em(symbol).iloc[-1]  ## 有估值指标需要的字段
#         if cap_series:
#             cap_data = cap_series.to_frame().T
#         else:
#             cap_data = ak.stock_individual_info_em(symbol)
...
#                 "float_capital": float(cap_data.get("流通股本", 0)),
#
#                 # 盈利数据
#                 "revenue": revenue,
...
#                 # 估值比率
#                 # 使用PE(TTM)作为市盈率指标
#                 "pe_ratio": float(cap_data.get("PE(TTM)", 0)),
#                 "price_to_book": float(cap_data.get("市净率", 0)),
#                 # "price_to_book": 0.0,
#                 "price_to_sales": (float(cap_data["总市值"].iloc[0]) / revenue) if revenue != 0 else 0.0
#             }
#
#             # 只返回 agent 需要的指标
#             agent_metrics = {
