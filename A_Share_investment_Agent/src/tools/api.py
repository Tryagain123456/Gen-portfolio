...
def get_financial_metrics(symbol: str) -> Dict[str, Any]:
    ...
    try:
        ...
        #     logger.warning(f"× {symbol} 没有可用的实时行情数据")
        #     return [{}]
        # stock_data = realtime_data[realtime_data['代码'].str[2:] == symbol]
        # if stock_data.empty:
        #     logger.warning(f"× {symbol} 没有找到实时行情数据")
        #     return [{}]
        # stock_data = stock_data.iloc[0]
        # logger.info(f"✓ {symbol} 的实时行情数据获取成功")
        ...
        logger.info(f"✓ {symbol} 的财务指标获取成功，共{len(financial_data)}条记录 (最后一条日期为{latest_financial.get('日期')})")
        ...
        try:
            ...
            all_metrics = {
                ...
                "float_market_cap": float(cap_data["流通市值"].iloc[0]),
                ...
                "price_to_book": 0.0,
                "price_to_sales": float(cap_data["总市值"].iloc[0]) / float(latest_income.get("营业总收入", 0))
            }
...
def get_market_data(symbol: str) -> Dict[str, Any]:
    """获取市场数据"""
    logger.info(f"获取股票代码为 {symbol} 的市场数据...")
    try:
        # 获取实时行情
        realtime_data = ak.stock_zh_a_spot()
        stock_data = realtime_data[realtime_data['代码'].str[2:] == symbol].iloc[0]
