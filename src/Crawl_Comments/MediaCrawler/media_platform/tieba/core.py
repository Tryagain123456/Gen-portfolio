    ...
    async def _navigate_to_tieba_via_baidu(self):
        ...
        try:
            ...
            if not tieba_link:
                ...
                # --- [新增] 手动介入 (针对未找到链接,直接访问的情况) ---
                utils.logger.warning("=" * 50)
                utils.logger.warning(
                    "[TieBaCrawler] 已打开贴吧页面(回退模式)，请在 60 秒内手动完成 [验证码] 或 [扫码登录] 操作...")
                utils.logger.warning("=" * 50)
                await self.context_page.wait_for_timeout(60000)  # 暂停 60 秒
                utils.logger.info("[TieBaCrawler] 手动操作时间结束，继续执行...")
                # --- [新增结束] ---
                return
            ...
            # --- [新增] 手动介入 (针对点击链接,成功跳转的情况) ---
            utils.logger.warning("=" * 50)
            utils.logger.warning(
                "[TieBaCrawler] 已打开贴吧页面(标准模式)，请在 60 秒内手动完成 [验证码] 或 [扫码登录] 操作...")
            utils.logger.warning("=" * 50)
            await self.context_page.wait_for_timeout(60000)  # 暂停 60 秒
            utils.logger.info("[TieBaCrawler] 手动操作时间结束，继续执行...")
            # --- [新增结束] ---
