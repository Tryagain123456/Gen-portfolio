...
def context_extract(platform: str, keywords: List[str]):
    ...
    try:
        ...
        # utils.logger.error(...) # 如果你有 logger 的话
        print(f"!!! 错误：爬虫未能成功生成结果文件: {data_contents_path}")
        print(f"!!! 请检查 MediaCrawler 的日志，确认 CRAWLER_TYPE 是否为 'search' 并且搜索是否成功。")
        return None  # 或者返回一个空列表/字典，防止主程序崩溃
    except json.JSONDecodeError:
        print(f"!!! 错误：爬虫结果文件已生成，但内容为空或格式错误: {data_contents_path}")
        return None
