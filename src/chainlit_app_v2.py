...
async def on_chat_start():
    ...
    await cl.Message(
        ...
        actions=[
            cl.Action(name="test_html_render", label="🧪 预测图样例展示",payload={})
        ]
    ).send()
...
async def handle_test_html(action):
    # [方案 B]
    # 我们假设文件现在位于 "public/output_images_kronos/"
    # 我们可以通过相对 URL "/public/output_images_kronos/..." 来访问它

    # 注意：我们仍然需要检查文件是否 *实际* 存在于文件系统上
    ...
    if public_file_path.exists():
        # 这个 URL 是相对于 Chainlit 服务器根目录的
        file_url = f"/output_images_kronos/{file_name}"
