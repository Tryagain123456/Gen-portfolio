...
报告格式说明如下：
1. 不用标题和落款日期，仅呈现上述5个方面的内容即可。
2. 不用markdown格式的标题、加粗等的格式输出
3. 每个方面的内容用 a) b) c) 作为序号表示
...
#"macro_news_agent", "获取沪深300全量新闻并进行宏观分析，为投资决策提供市场层面的宏观环境评估")
def macro_news_agent(state: AgentState) -> Dict[str, Any]:
    ...
    show_workflow_status("Macro News Agent")
    ...
    show_workflow_status("Macro News Agent", "completed")
