...
# 设置日志记录
logger = setup_logger('macro_market_agent')

#"macro_analyst", "宏观分析师，分析宏观经济环境对目标股票的影响")
def macro_market_agent(state: AgentState):
    """Responsible for macro analysis"""
    show_workflow_status("macro_market_agent")
    ...
    message = HumanMessage(
        ...
        name="macro_market_agent",
    )

    show_workflow_status("macro_market_agent", "completed")
    # logger.info(f"--- DEBUG: macro_market_agent COMPLETED ---")
    # logger.info(
    # f"--- DEBUG: macro_market_agent RETURN messages: {[msg.name for msg in (state['messages'] + [message])]} ---")
    return {
...
def get_macro_news_analysis(news_list: list) -> dict:
    ...
    # 检查缓存
    import os
    cache_file = "data/macro_analysis_cache.json"
