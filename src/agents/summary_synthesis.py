...
##### (1) 相关行业角度
宏观环境信号: {signal_to_chinese(general_macro_signal.get('macro_environment', '无数据') if general_macro_signal else '无数据')} & 对股票影响信号: {signal_to_chinese(general_macro_signal.get('impact_on_stock', '无数据') if general_macro_signal else '无数据')}
关键因素: 
{';'.join(general_macro_signal.get('key_factors', ['无数据']) if general_macro_signal else ['无数据'])}
关键因素分析: 
【相关行业角度关键因素分析待补充】
...
def summary_synthesis_agent(state: AgentState):
    ...
    1. 基于'基本面分析'、'估值分析'、'技术分析'、'风险评估'、'宏观环境分析中相关行业角度'原始报告数据，各团队分析依据和结果，对这四个模块生成 3-4 句情况分析结论；
    ...
    1. 将 任务一步骤2 生成的最终投资建议（买入/卖出/持有）写入【操作建议待补充】部分，注意写入内容表述为以下3个选项中选一个
        ...
        - 继续持有/场外观望 ➡️】
    2. 参考 任务一步骤1 中五个模块的情况分析结论，补充原始报告中【基本面情况分析待补充】、【估值情况分析待补充】、【技术面情况分析待补充】、【风险情况分析待补充】、【相关行业角度关键因素分析待补充】中的内容；
    3. 将 任务一步骤3 中的结论写入原始报告中【决策分析汇总待补充】，必要时可以分点作答，用无序列表 - 符号表示
    4. 优化原始报告中 “股票走势分析”、“4.网络情绪分析”、“5.宏观环境分析” 的语句表达，在不改变核心表述的前提下提升流畅度
    注意在补充报告时，请严格遵循以下要求：
    ...
    - 宏观环境分析团队：(1)相关行业角度是{tool_based_macro_content}; (2)大盘新闻角度是{market_wide_news_summary_content}
