...
#     intent_router,   # 一个返回下一个节点名的函数
#     {
#         "market": "market_data_tool",
...
#             file_path = (
#                         base_dir / "output_images_kronos" / f"{stock_ticker}_{current_date_str}_pred_90d.html").resolve()
        ...
        if intent == "stock_analysis":
            ...
            try:
                async with async_playwright() as p:
                    ...
                    file_uri = file_path.as_uri()
                    await page.goto(file_uri, wait_until="networkidle")  # Traceback 中的错误发生在这里

                    await page.screenshot(path=str(png_path), full_page=True)
                    await browser.close()  # 及时关闭浏览器

                # 发送带图片的最终消息
                img_element = cl.Image(path=str(png_path), name=f"{stock_ticker}_pred_90d.png", display="inline")
                await cl.Message(content=result_content + "\n\n #### 📊 以下是该股票未来90日的预测图：\n\n",
                                 elements=[img_element]).send()
